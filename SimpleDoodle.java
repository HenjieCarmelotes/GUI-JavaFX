...
public class SimpleDoodle extends Application {
     ...
     * "Clear" button on the bottom right edge of the canvas.  This method is called when
     * the canvas is created 2 times: First when the canvas is created in GUI setup, 
     * second when the user clicks "Clear."
     */
    public void drawPaletteAndClear() {
        ...
        int colorSpacing = (width - 56) / 7; //width minus the pixel space for clear button divided by 7 colored rects.
     ...
     for (int N = 0; N < 7; N++) {
         g.setFill( palette[N] );
         g.fillRect(3 + N*colorSpacing, height-53, colorSpacing-3, 50);
     }
	...
	g.setLineWidth(2);
	g.strokeRect(2 + currentColorNum*colorSpacing, height-54, colorSpacing-1, 52);
        
    } //end of drawPaletteAndClear()
    ...
    private void changeColor(int x) {
        ...
        int colorSpacing = (width - 56) / 7;  // Space for one color rectangle.
        int newColor = x / colorSpacing;       // Which color number was clicked?
    ...
    public void mousePressed(MouseEvent evt) {
        ...
        if (y > height - 53) {
            // User clicked to the bottom of the drawing area.
            // This click is either on the clear button or
            // on the color palette.
            if (x > width - 53)
            	drawPaletteAndClear();  //  Clicked on "CLEAR button".
            else
                changeColor(x);  // Clicked on the color palette.
        }
        else if (y > 3 && y < height - 56 && x > 3 && x < width - 3) {
    ...
    public void mouseDragged(MouseEvent evt) {
        ...
        if (x > canvas.getWidth() - 4)       //   the drawing area.
            x = (int)canvas.getWidth() - 4;

        if (y < 3)                          // Adjust the value of y,
            y = 3;                           //   to make sure it's in
        if (y > canvas.getHeight() - 57)       //   the drawing area.
            y = canvas.getHeight() - 57;
    ...
    public void start(Stage stage) {
        ...
        canvas = new Canvas(400,600); //The canvas has been created. Any change with the size of the canvas 
        			//will not affect the created canvas already.
        ...
        //we can modify the lambda notation into method reference since the event handler  
        //calls a method that has the same parameter as the event handler.
        ...
        StackPane top = new StackPane(quitButton);
        top.setStyle("-fx-background-color: gray; -fx-padding:5px;" + 
